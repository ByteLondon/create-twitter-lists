...
const inputFile = 'user_locations.csv'
const asyncTasks = [],
  examplesDir = 'examples/locations/'
...
const parser = parse({ delimiter: ',' }, function (err, users) {
  const asyncTasks: Function[] = [],
    listsObj = {},
    noLocationUsers: string[] = [],
    validAddressUsers: string[] = [],
    allUsers: string[] = []
  const googleMapsClient = googleMaps.createClient(config.auth.google)
  const noLocationWriter = csvWriter({ sendHeaders: false })
  noLocationWriter.pipe(fs.createWriteStream(examplesDir + 'noLocation.csv'))

  users.forEach(function (user, i) {
    const handle = user[0] || null
    const location = user[1] || null
        ...
        function (err, response) {
          if (err) {
            ...
            if (response.json.results[0]) {
              const addressComponents = response.json.results[0].address_components || null
              if (addressComponents) {
                for (const i in addressComponents) {
                  const component = addressComponents[i]
                  if (component.types.indexOf('country') >= -1) {
                    if (
                      countryContinentMapping[component['short_name']] !== undefined &&
                      countryContinentMapping[component['short_name']]
                    ) {
                      const continent = countryContinentMapping[component['short_name']]
  ...
  async.series(asyncTasks, function (err) {
    ...
    const invalidAddressUsers = allUsers
      .filter((x) => noLocationUsers.indexOf(x) < 0)
      .filter((x) => validAddressUsers.indexOf(x) < 0)
    const invalidAddressWriter = csvWriter({ sendHeaders: false })
    ...
    const parallelTasks: Function[] = []
    Object.keys(listsObj).forEach(function (continent) {
      const members = listsObj[continent].map(function (handle) {
        return handle.replace('@', '').trim()
      })
      parallelTasks.push((callback) => {
        const writer = csvWriter({ sendHeaders: false, newline: ',' })
        const fileName = examplesDir + continent.replace(/ /g, '_') + '.csv'
